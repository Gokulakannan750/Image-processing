...
class PerformanceMonitor:
    ...
    def tick(self) -> None:
        ...
        if len(self.frame_times) > 0:
            ...
            if self.current_fps < self.min_fps_warning:
                log.warning(
                    "Performance degraded! FPS: %.1f (Min: %.1f)",
                    self.current_fps,
                    self.min_fps_warning,
                )
    ...
    def draw_overlay(self, frame: np.ndarray) -> None:
        ...
        cv2.putText(frame, text, (w - 150, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
        ...
        if self.show_graphs and len(self.latency_times) > 2:
            self._draw_mini_graph(
                frame, self.latency_times, (w - 160, h - 60), (150, 50)
            )
    ...
    def _draw_mini_graph(
        ...
        frame[y_off : y_off + gh, x_off : x_off + gw] = cv2.addWeighted(
            sub_img, 0.7, rect, 0.3, 0
        )
