...
class DetectionResult:
    ...
    def primary_target(self) -> Optional[DetectionTarget]:
        if not self.targets:
            return None
        # Sort by priority first, then distance (closest first if distance exists)
...
class BaseDetector(ABC):
    ...
    def process_frame(
        ...
        Returns:
            A 2-tuple of:
            - **annotated_frame**: The frame with overlays drawn.
            - **DetectionResult**: Structured object containing all targets.
        """
        ...
